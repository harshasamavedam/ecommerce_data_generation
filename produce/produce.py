...
from .db_connect import insert_record, create_new_table, read_records

f = Faker()
def production():
   print("Starting production process...")
   events=["plp_view","pdp_view","add_to_cart","item_viewed","item_checked"]
   events_after_50=events+["checkout_btn_clicked","payment_login_redirect","checkout_started","purcahse"]
   ...
   for j in range(no_of_users):
       ...
       for i in range(max_events): 
            ...
            data={"User_ID": user_id, "Name": f.name(), "Address": f.address(), "Email": f.email(), "Event": random.choice(events_choice),"timestamp":datetime.now()}
            try:
                insert_record("events_hourly", ["id", "name", "email","address","event","timestamp"], [str(data["User_ID"]), data["Name"], data["Email"], data["Address"], data["Event"], data["timestamp"]])
